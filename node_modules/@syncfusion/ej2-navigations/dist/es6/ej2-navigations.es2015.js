...
let Toolbar = class Toolbar extends Component {
    ...
    popupInit(element, ele) {
        if (!this.popObj) {
            ...
            let eleStyles = window.getComputedStyle(element);
...
let ContextMenu = class ContextMenu extends Component {
    ...
    createItems(items) {
        ...
        let ul = ListBase.createList(this.createElement, this.toRawObject(items.slice()), listBaseOptions, true);
    ...
    getIndex(data, isUniqueId, items = this.items, navIdx = [], isCallBack = false) {
        let item;
        for (let i = 0, len = items.length; i < len; i++) {
            item = items[i];
        ...
        if (typeof args === 'number') {
            ...
            if (this.tbItem.length > args && args >= 0 && !isNaN(args)) {
                if (!this.tbItem[this.selectedItem].classList.contains('e-hidden')) {
                    ...
                    for (let i = this.selectedItem; i >= 0; i--) {
                        if (!this.tbItem[i].classList.contains('e-hidden')) {
                            this.prevIndex = i;
                            break;
                        }
...
let TreeView = TreeView_1 = class TreeView extends Component {
    ...
    getDragEvent(event, obj, dropTarget, target, dragNode) {
        ...
        let newParent = dropTarget ? this.parents(dropTarget, '.e-list-item') : null;
    ...
    addGivenNodes(nodes, dropLi, index, isRemote) {
        ...
        let li = ListBase.createListItemFromJson(this.createElement, nodes, this.listBaseOption, level);
...
TreeView = TreeView_1 = __decorate$5([
    NotifyPropertyChanges
], TreeView);
var TreeView_1;
